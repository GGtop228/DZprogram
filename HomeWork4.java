...
public class HomeWork4 {
    ...
    /**
     * Задание №2
     * Ивана сортирует в магазине свою корзину покупок.i
    ...
    public static List<String> correctedList(List<String> stringList) {
        ...
        for (String bro:stringList){
            int upB=0;//заглавные буквы
            int probel = 0;//пробелы (почему-то пробелы считаются заглавными)
            String cap = bro.toUpperCase();

            for (int i=0;i<bro.length();i++){
                if (bro.charAt(i)==' '){//считаю пробелы
                    probel+=1;
                }
                if (cap.charAt(i) == bro.charAt(i)){//считаю заглавные буквы
                    upB+=1;}}

            if ((upB-probel)==0){//проверяю если кол-во заглавных = 0 то записываю в массив
                slova.add(bro);
            }
    ...
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
        ...
        HashMap<String, Integer> shopCart2 = new HashMap<>();
        for (Map.Entry<String, Integer> elem : shopCart.entrySet()){
            if (elem.getValue()<=300) shopCart2.put(elem.getKey(),elem.getValue());
