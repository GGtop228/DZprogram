...
public class  HomeWork3 {
    ...
    public static void raiseException(Integer exceptionId) throws Exception,SocketException,ClassNotFoundException,AccessDeniedException,ExportException{ // Подсказка: throws Exception в итоге надо поменять на перечисление имен исключений
        if (exceptionId==1) throw new SocketException();
        if (exceptionId==2) throw new ClassNotFoundException();
        if (exceptionId==3) throw new AccessDeniedException("ошибка");
        if (exceptionId==4) throw new ExportException("ошибка");
